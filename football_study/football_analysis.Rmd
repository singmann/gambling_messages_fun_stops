...
We first load in the data from all participants and transform variables into correct type. We also perform some checks on the data to confirm that it mateches our expectations (i.e., in size and structure).
...
bets <- read_csv("data/bets_data.csv") %>% 
  ...
  filter(!(pptid %in% ids_exclude)) %>% 
  droplevels()
colnames(bets) <- tolower(colnames(bets))

stopifnot(length(levels(bets$pptid)) == 1003)
...
```{r, fig.asp=1.0}
plot(mzoib, pars = "conditionTreatment")
```
...
pp1
ggsave(plot = pp1, filename = "ppp.png", width = 8, height = 6)
...
lpars <- mzoib %>% 
  ...
  mutate(type = case_when(
    ...
    str_detect(.variable, "conditionTreatment") ~ "effect",
  )) %>%  
...
```{r, out.width="100%", fig.width=8, fig.asp=0.8, warning=FALSE}
comppars %>% 
  ggplot() +
...
We see that even with an extremely small prior with SD = 0.05 we still see evidence for no difference (i.e., black density estimate above the prior at 0) for all but the lower right plot. Note that a prior with SD of .05 means that we expect with 95% that the largest effect we observe is `r scales::percent(plogis(0.05 *2) - 0.5, accuracy = 0.01)` on the response scale.
...
We can also plot the three parameters showing the difference distribution of the message condition from the no messafe condition. These differences are given on the logit scale.

```{r, fig.asp=1.0}
plot(mzoib2, pars = "conditionTreatment")
```
...
ggsave(filename = "cov_plot.png", plot = cvp, width = 6.5, height = 4)
...
mzoib3 <- brm(formula = zoib_model3, data = part2, stanvars = stanvars, prior = np)
save(mzoib3, file = "model_zoib3.rda", compress = "xz")
```


```{r}
load("model_zoib3.rda")
summary(mzoib3)
```
...
mrisk2 <- brm(formula = risk_frml2, data = bets_av, family = Beta(), 
             chains = 4, cores = 4)
save(mrisk2, file = "model_risk2.rda", compress = "xz")
```
...
mrisk <- brm(formula = risk_frml, data = bets_av, family = zero_one_inflated_beta(), 
             chains = 4, cores = 4)
save(mrisk, file = "model_risk.rda", compress = "xz")
```
