...
Age distribution (first number is N of NAs):

```{r}
part_excl <- captchas_all %>% 
  filter(pptid %in% ids_use$pptid)
sum(is.na(part_excl$age))
psych::describe(part_excl$age)
```

```{r}
table(part_excl$Sex, useNA = "always")
prop.table(table(part_excl$Sex, useNA = "always"))
```
...
part2 %>% 
  summarise(no_gamble = sum(new_prop  == 0),
            gamble_at_all = sum(new_prop  != 0), 
            gamble_everything = sum(new_prop[new_prop != 0] == 1))
